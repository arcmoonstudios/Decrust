...
class CrateChecker:
    ...
    def metadata_checks(self) -> bool:
        ...
        required_files = [
            ...
            ("LICENSE.md", "License file (LICENSE.md)"),
        ]
    ...
    def print_summary(self):
        ...
        if not self.failed_checks:
            ...
            print(f"{Colors.GREEN}All critical checks passed. You can proceed with publishing.{Colors.END}")
            if self.warnings:
                print(f"{Colors.YELLOW}Consider addressing the warnings for better quality.{Colors.END}")
            return True
        else:
            print(f"\n{Colors.RED}{Colors.BOLD}🚫 CRATE NOT READY FOR RELEASE{Colors.END}")
            print(f"{Colors.RED}Please fix the critical failures before publishing.{Colors.END}")
            return False
